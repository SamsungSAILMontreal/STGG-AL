...
import subprocess
import numpy as np
from mol_to_coord import MolToCoord
from rdkit import Chem

__all__ = ["xtb4stascore"]
...
default_stda_config = {
    'log_dir': "CHANGE_TO_YOUR_DIR/stda_scratch2", # CHANGE DIRECTORY TO YOUR OWN
    'xtb_path': "CHANGE_TO_YOUR_DIR/xtb4stda", # CHANGE DIRECTORY TO YOUR OWN
    'stda_command': "stda_v1.6.3",
    "moltocoord_config": {
        'log_dir': "CHANGE_TO_YOUR_DIR/mol_coords_logs2", # CHANGE DIRECTORY TO YOUR OWN
        "final_coords_log_dir": "CHANGE_TO_YOUR_DIR/final_mol_coords2", # save all final coordinates to here # CHANGE DIRECTORY TO YOUR OWN
...
class STDA_XTB:
    ...
    def get_scores(self, molecule_list, **kwargs):
        ...
        for i, mol in enumerate(molecule_list):
            print(mol)
            new_dict = self.get_score(mol, **kwargs)
            prop[i,0] = new_dict['energy']
            prop[i,1] = new_dict['f_osc']
        os.chdir(self.current_dir)
        return prop

    def get_scores_mp(self, molecule_list, **kwargs): # not faster
        pool = multiprocessing.Pool(multiprocessing.cpu_count())
        prop = np.zeros((len(molecule_list), 2))
        score_ = partial(self.get_score_, **kwargs)
        prop = pool.map(score_, molecule_list)
        return np.vstack(prop)
    ...
    def _get_lambda_osc(self, coord_file):
        ...
        try:
            ...
            energy, wavelength, fosc = 0.0, 0.0, 0.0
...
class xtb4stascore:
    def __init__(self, constraint="IR"):
        cfg = default_stda_config
        self.stdaxtb = STDA_XTB(**cfg)
        self.constraint = constraint

    def __call__(self, smiles) -> np.ndarray:
        print(smiles)
        mols = [Chem.MolFromSmiles(smile) for smile in smiles]
        props = self.stdaxtb.get_scores(smiles)
