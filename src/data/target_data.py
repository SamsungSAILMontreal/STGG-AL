...
class SpanningTreeVocabulary:
    ...
    def smiles2molgraph(self, smiles, randomize_atom_order=False):
        ...
        G = nx.Graph()
        #print("smiles-to-graph")
        #print(smiles)
        for atom in mol.GetAtoms():
            #print(f"{self.ATOMFEAT2TOKEN[(atom.GetAtomicNum(), atom.GetFormalCharge(), atom.GetNumExplicitHs())]} {(atom.GetAtomicNum(), atom.GetFormalCharge(), atom.GetNumExplicitHs())}")
    ...
    def molgraph2smiles(self, G):
        ...
        mol = Chem.RWMol()
        node_to_idx = dict()
        #print("graph-to-smiles")
        for node in G.nodes():
            #print(f"{node_tokens[node]}  {self.TOKEN2ATOMFEAT[node_tokens[node]]}")
...
class Data:
    def __init__(self, MAX_LEN, vocab, n_correct=0, allow_empty_bond=True, tracker_id=0,
        no_repeated_higher_bond=False, banned_tokens=[]): 
        self.sequence = []
        self.tokens = []
        ...
        self.no_repeated_higher_bond = no_repeated_higher_bond
        self.banned_tokens = banned_tokens # tokens that are BANNED during generation
    ...
    def update(self, id):
        ...
        if must_end_soon: # we must close the Spanning-tree ASAP (Note that branches must all be closed, but rings can be left open)
            ...
            #print(self.n_correct)
            #print(token)
            #print(len(self.branch_start_nodes))
            ...
            if token in self.vocab.ATOM_TOKENS:
                ...
                if self.no_repeated_higher_bond: # cannot repeat a double-bond
                    allowed_next_tokens = list(set(allowed_next_tokens) - set(BOND_TOKENS[self.prev_higher_bond]))
        ...
        allowed_next_tokens = [token for token in allowed_next_tokens if token not in self.banned_tokens] # we remove the banned tokens

        # check if repeating double or triple bond (e.g., =C=)
        if token in "=":
            self.prev_higher_bond = 1
        elif token in "#":
            self.prev_higher_bond = 2
        elif token in "$":
            self.prev_higher_bond = 3
        if token not in self.vocab.ATOM_TOKENS + [RING_START_TOKEN] + [BRANCH_START_TOKEN]:
            self.prev_higher_bond = 0
    ...
    def from_smiles(smiles, vocab, randomize_order=False, MAX_LEN=250, start_min=True):
        ...
        #print(f"n_nodes={len(molgraph.nodes)}")
        #print(f"len_tokens={len(tokens)}")
        mollinegraph = nx.Graph()
