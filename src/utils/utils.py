...
def property_loss(batched_sequence_data, prop, pred_prop, cont_var_index, cat_var_index, 
    lambda_predict_prop=1.0, lambda_predict_prop_always=False, bin_loss_type='ce'):
    L = pred_prop.shape[1]

    if lambda_predict_prop_always:
        if len(cont_var_index) > 0:
        ...
        if len(cat_var_index) > 0:
            ...
            elif bin_loss_type == 'L2':
                y = 2*prop[:, cat_var_index] - 1 # [0,1] -> [-1,+1]
                loss_prop_cat += lambda_predict_prop*0.5*(((pred_prop[:, :, cat_var_index] - y.unsqueeze(1))**2).sum(2).mean(dim=1)).mean(dim=0)
            elif bin_loss_type == 'hinge':
                y = 2*prop[:, cat_var_index] - 1 # [0,1] -> [-1,+1]
                loss_prop_cat += lambda_predict_prop*(torch.nn.ReLU()((1.0 - pred_prop[:, :, cat_var_index])*y.unsqueeze(1))).sum(2).mean()
            else:
                raise NotImplementedError()
    else:
        eos = batched_sequence_data == EOS_TOKEN_id # b x l
        loss_prop_cont = lambda_predict_prop*0.5*(((pred_prop[eos][:, cont_var_index] - prop[:, cont_var_index])**2).sum(1)).mean(dim=0)
        loss_prop_cat = 0.0
        for i in cat_var_index:
            loss_prop_cat += lambda_predict_prop*bce_loss(pred_prop[eos][:, i], prop[:, i])
...
def compute_sequence_cross_entropy(logits, batched_sequence_data, ignore_index=0, prop=None, pred_prop=None, 
    lambda_predict_prop=0.0, lambda_predict_prop_always=False, 
    cont_var_index=None, cat_var_index=None, bin_loss_type='ce'):
    logits = logits[:, :-1]
    targets = batched_sequence_data[:, 1:]
    ...
    if lambda_predict_prop > 0:
        ...
        loss_prop_cont, loss_prop_cat = property_loss(batched_sequence_data=batched_sequence_data, prop=prop, pred_prop=pred_prop, 
            ...
            lambda_predict_prop=lambda_predict_prop, lambda_predict_prop_always=lambda_predict_prop_always,
